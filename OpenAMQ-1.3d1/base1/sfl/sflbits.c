    ...
    for (block_nbr = 0; block_nbr < bits-> block_count; block_nbr++)
      {
        ...
        fwrite (&block_ptr-> right, sizeof (block_ptr-> right), 1, file);
        fwrite (&block_ptr-> size,  sizeof (block_ptr-> size),  1, file);

        comp_size = compress_block (block_ptr-> block.data,
                                    compressed, (word)BIT_DATASIZE);

        fwrite (&comp_size, sizeof (comp_size), 1,         file);
        fwrite (compressed, 1,                  comp_size, file);
      }
    return 0;
}
    ...
    for (block_nbr = 0; block_nbr < bits-> block_count; block_nbr++)
      {
        ...
        block_ptr        = bits-> block [block_nbr];
        fread (&block_ptr-> right, sizeof (block_ptr-> right), 1,         file);
        fread (&block_ptr-> size,  sizeof (block_ptr-> size),  1,         file);
        fread (&comp_size,         sizeof (comp_size),         1,         file);
        fread (compressed,         1,                          comp_size, file);
