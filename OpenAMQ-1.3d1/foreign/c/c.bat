...
:msvc_go
    ...
    if !_MT!==0 (
        if !_DEBUG!==1  set _CC=!_CC! /MLd /Z7 /D"DEBUG" /Od
        if !_DEBUG!==0  set _CC=!_CC! /ML
    ) else (
        if !_DEBUG!==1  set _CC=!_CC! /MTd /Z7 /D"DEBUG" /Od
        if !_DEBUG!==0  set _CC=!_CC! /MT
        set _CC=!_CC! /D"BASE_THREADSAFE"
    )
...
:msvc_link
    ...
    if !_MT!==0 (
        if !_DEBUG!==1 (
            echo !_CCLIBDIR!libcd.lib   >>library.lst
            if !_USECPP!==1 echo !_CCLIBDIR!libcpd.lib  >>library.lst
        ) else (
            echo !_CCLIBDIR!libc.lib   >>library.lst
            if !_USECPP!==1 echo !_CCLIBDIR!libcp.lib  >>library.lst
        )
    ) else (
        if !_DEBUG!==1 (
