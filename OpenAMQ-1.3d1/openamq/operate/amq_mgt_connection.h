...
{\
    if (!self) {\
        ...
        fprintf (stderr, "Attempting to work with a NULL amq_mgt_connection\n");\
    ...
    if (self->object_tag != AMQ_MGT_CONNECTION_ALIVE) {\
        fprintf (stderr, "FATAL ERROR at %s:%u, in %s\n", __FILE__, __LINE__, ICL_ASSERT_SANE_FUNCTION);\
        fprintf (stderr, "amq_mgt_connection at %p expected object_tag=0x%x, actual object_tag=0x%x\n", self, AMQ_MGT_CONNECTION_ALIVE, self->object_tag);\
