/*---------------------------------------------------------------------------
    amq_vhost_agent.c - functions for amq_vhost_agent.
...
#define destroy_m ((amq_vhost_agent_destroy_t *) thread->method->data)

#define unbind_queue_m ((amq_vhost_agent_unbind_queue_t *) thread->method->data)

enum {
    shutdown_event                                 = 1,
    destroy_m_event                                = 2,
    unbind_queue_m_event                           = 3,
};
...
#define amq_vhost_agent_agent_name                 "Amq-Vhost-Agent"
...
amq_vhost_agent_destroy_t;

typedef struct
  {
    amq_queue_t *  queue;
  }
amq_vhost_agent_unbind_queue_t;
...
} amq_vhost_agent_class_context_t;
...
#if (defined (BASE_ANIMATE) || defined (BASE_ANIMATE_AMQ_VHOST_AGENT)          ||  defined (BASE_TRACE)  || defined (BASE_TRACE_AMQ_VHOST_AGENT)   ||  defined (AMQ_VHOST_AGENT_TRACE))
...
#if (defined (BASE_ANIMATE) || defined (BASE_ANIMATE_AMQ_VHOST_AGENT))
static Bool
    s_animate = FALSE;
#endif

#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT_DESTROY))
static icl_stats_t *s_amq_vhost_agent_destroy_stats = NULL;
#endif
#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT_FREE))
static icl_stats_t *s_amq_vhost_agent_free_stats = NULL;
#endif
#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT_UNNAMED_3))
static icl_stats_t *s_amq_vhost_agent_unnamed_3_stats = NULL;
#endif
#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT_UNNAMED_4))
static icl_stats_t *s_amq_vhost_agent_unnamed_4_stats = NULL;
#endif
#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT_UNBIND_QUEUE))
static icl_stats_t *s_amq_vhost_agent_unbind_queue_stats = NULL;
#endif
#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT_UNNAMED_6))
static icl_stats_t *s_amq_vhost_agent_unnamed_6_stats = NULL;
#endif
...
static int    amq_vhost_agent_manager         (smt_thread_t **thread);
static int    amq_vhost_agent_catcher         (smt_thread_t *thread);
static int    amq_vhost_agent_class_destroy   (smt_thread_t *thread);
...
#endif
#if (defined (BASE_TRACE)   || defined (BASE_TRACE_AMQ_VHOST_AGENT)            ||  defined (AMQ_VHOST_AGENT_TRACE))
...
static int
amq_vhost_agent_destroy_send (
    smt_method_queue_t *_queue,
    smt_event_t        _event,
    char *         file,
    size_t         line);
int
amq_vhost_agent_destroy_destructor (void *data);
static int
amq_vhost_agent_unbind_queue_send (
    smt_method_queue_t *_queue,
    smt_event_t        _event,
    amq_queue_t *  queue);
int
amq_vhost_agent_unbind_queue_destructor (void *data);
    ...
    size_t         line)
{
    ...
    rc = smt_method_send (_queue,           //  The queue
                          ...
                          amq_vhost_agent_destroy_destructor);
...
size_t line;
    ...
    file = ((amq_vhost_agent_destroy_t *) data)-> file;
    line = ((amq_vhost_agent_destroy_t *) data)-> line;
    ...
    amq_queue_t *  queue)
{
    ...
    rc = smt_method_send (_queue,           //  The queue
                          ...
                          SMT_OK,           //  Always OK
                          _message,         //  The message
                          amq_vhost_agent_unbind_queue_destructor);
...
amq_queue_t * queue;
    ...
    queue = ((amq_vhost_agent_unbind_queue_t *) data)-> queue;
    ...
    size_t                line)
{
    ...
    if (thread && !thread->zombie) {
        assert (thread->manager == amq_vhost_agent_manager);
        rc = amq_vhost_agent_destroy_send (
    ...
    amq_queue_t *         queue)
{
    ...
    if (thread && !thread->zombie) {
        assert (thread->manager == amq_vhost_agent_manager);
        rc = amq_vhost_agent_unbind_queue_send (
...
void
amq_vhost_agent_term (void)
{

}

int 
amq_vhost_agent_init (void)
{
    int
        rc = SMT_OK;                    //  Return code

    s_class_context_cache = icl_cache_get (sizeof (amq_vhost_agent_class_context_t));
    s_destroy_cache = icl_cache_get (sizeof (amq_vhost_agent_destroy_t));
    s_unbind_queue_cache = icl_cache_get (sizeof (amq_vhost_agent_unbind_queue_t));
    ...
    icl_system_register (NULL, amq_vhost_agent_term);
    
    return rc;
}

void
amq_vhost_agent_animate (Bool animate)
{
#if (defined (BASE_ANIMATE) || defined (BASE_ANIMATE_AMQ_VHOST_AGENT))
    s_animate = animate;
#endif
...
smt_thread_t *
amq_vhost_agent_class_thread_new (
    amq_vhost_t * self)
{
    smt_thread_t
        *thread;

    thread = smt_thread_new (amq_vhost_agent_manager,
                             amq_vhost_agent_catcher,
                             amq_vhost_agent_class_destroy);
                             

#if (defined (AMQ_VHOST_AGENT_TRACE))
    ...
    if (thread) {
        ...
        memset (thread->context, 0, sizeof (amq_vhost_agent_class_context_t));
        ((amq_vhost_agent_class_context_t *) thread->context)->links = 1;
        thread->event_name = event_name;
{
#define tcb ((amq_vhost_agent_class_context_t *) thread->context)
...
static int
amq_vhost_agent_manager (smt_thread_t **thread_p)
{
    smt_thread_t
        *thread = *thread_p;
...
#undef  tcb
            ...
            switch (thread->_next_event) {
                ...
                case shutdown_event:
#if (defined (BASE_TRACE) || defined (BASE_TRACE_AMQ_VHOST_AGENT))
...
static int
amq_vhost_agent_catcher (smt_thread_t *thread)
{
    return SMT_OK;
}
...
static int    
amq_vhost_agent_class_destroy (smt_thread_t *thread)
{
#if (defined (BASE_TRACE) || defined (BASE_TRACE_AMQ_VHOST_AGENT))
...
#endif
    if (--((amq_vhost_agent_class_context_t *) thread->context)->links == 0)
        ...
        else
          {
            *state_id = 2;
            switch (thread->place) {
                ...
                case 8: *event_id = 3;   *nextstate_id = 2;   break;
                case 9: *event_id = 1;   break;
            }
          }
      }
}
...
report_unrecognised_event_error (smt_thread_t *thread)
{
    ...
    icl_console_print ("amq_vhost_agent: Unrecognised event '%s' in '%s' thread number: %u, state: %s",
                       event_name_or_number,
...
#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT_DESTROY))
    icl_stats_inc ("amq_vhost_agent_destroy", &s_amq_vhost_agent_destroy_stats);
#endif        
#define tcb ((amq_vhost_agent_class_context_t *) thread->context)
{
...
#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT_FREE))
    icl_stats_inc ("amq_vhost_agent_free", &s_amq_vhost_agent_free_stats);
#endif        
#define tcb ((amq_vhost_agent_class_context_t *) thread->context)
{
...
#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT_UNNAMED_3))
    icl_stats_inc ("amq_vhost_agent_unnamed_3", &s_amq_vhost_agent_unnamed_3_stats);
#endif        
#define tcb ((amq_vhost_agent_class_context_t *) thread->context)
{
...
#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT_UNNAMED_4))
    icl_stats_inc ("amq_vhost_agent_unnamed_4", &s_amq_vhost_agent_unnamed_4_stats);
#endif        
#define tcb ((amq_vhost_agent_class_context_t *) thread->context)
{
...
#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT_UNBIND_QUEUE))
    icl_stats_inc ("amq_vhost_agent_unbind_queue", &s_amq_vhost_agent_unbind_queue_stats);
#endif        
#define tcb ((amq_vhost_agent_class_context_t *) thread->context)
{
...
#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT)  ||  defined (BASE_STATS_AMQ_VHOST_AGENT_UNNAMED_6))
    icl_stats_inc ("amq_vhost_agent_unnamed_6", &s_amq_vhost_agent_unnamed_6_stats);
#endif        
#define tcb ((amq_vhost_agent_class_context_t *) thread->context)
{
