...
{\
    if (!self) {\
        ...
        fprintf (stderr, "Attempting to work with a NULL amq_queue\n");\
    ...
    if (self->object_tag != AMQ_QUEUE_ALIVE) {\
        fprintf (stderr, "FATAL ERROR at %s:%u, in %s\n", __FILE__, __LINE__, ICL_ASSERT_SANE_FUNCTION);\
        fprintf (stderr, "amq_queue at %p expected object_tag=0x%x, actual object_tag=0x%x\n", self, AMQ_QUEUE_ALIVE, self->object_tag);\
...
volatile amq_queue_by_vhost_t *by_vhost_head;
volatile amq_queue_t *by_vhost_prev;
volatile amq_queue_t *by_vhost_next;
qbyte
...
#   endif
//  Method prototypes
#define amq_queue_new(vhost,connection,name,durable,exclusive,auto_delete,arguments)  amq_queue_new_ (__FILE__, __LINE__, vhost, connection, name, durable, exclusive, auto_delete, arguments)
amq_queue_t *
    amq_queue_new_ (
...
#define amq_queue_new_in_scope(self_p,_scope,vhost,connection,name,durable,exclusive,auto_delete,arguments)  amq_queue_new_in_scope_ (self_p, _scope, __FILE__, __LINE__, vhost, connection, name, durable, exclusive, auto_delete, arguments)
void
