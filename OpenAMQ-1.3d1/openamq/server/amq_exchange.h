...
{\
    if (!self) {\
        ...
        fprintf (stderr, "Attempting to work with a NULL amq_exchange\n");\
    ...
    if (self->object_tag != AMQ_EXCHANGE_ALIVE) {\
        fprintf (stderr, "FATAL ERROR at %s:%u, in %s\n", __FILE__, __LINE__, ICL_ASSERT_SANE_FUNCTION);\
        fprintf (stderr, "amq_exchange at %p expected object_tag=0x%x, actual object_tag=0x%x\n", self, AMQ_EXCHANGE_ALIVE, self->object_tag);\
...
#   include "amq_exchange_by_vhost.h"
...
volatile amq_exchange_by_vhost_t *by_vhost_head;
volatile amq_exchange_t *by_vhost_prev;
volatile amq_exchange_t *by_vhost_next;
qbyte
...
#   endif
//  Method prototypes
#define amq_exchange_new(table,vhost,type,name,internal,auto_federate)  amq_exchange_new_ (__FILE__, __LINE__, table, vhost, type, name, internal, auto_federate)
amq_exchange_t *
    amq_exchange_new_ (
...
#define amq_exchange_new_in_scope(self_p,_scope,table,vhost,type,name,internal,auto_federate)  amq_exchange_new_in_scope_ (self_p, _scope, __FILE__, __LINE__, table, vhost, type, name, internal, auto_federate)
void
