...
{\
    if (!self) {\
        ...
        fprintf (stderr, "Attempting to work with a NULL amq_consumer_by_channel\n");\
    ...
    if (self->object_tag != AMQ_CONSUMER_BY_CHANNEL_ALIVE) {\
        fprintf (stderr, "FATAL ERROR at %s:%u, in %s\n", __FILE__, __LINE__, ICL_ASSERT_SANE_FUNCTION);\
        fprintf (stderr, "amq_consumer_by_channel at %p expected object_tag=0x%x, actual object_tag=0x%x\n", self, AMQ_CONSUMER_BY_CHANNEL_ALIVE, self->object_tag);\
