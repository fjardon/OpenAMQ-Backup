...
{\
    if (!self) {\
        ...
        fprintf (stderr, "Attempting to work with a NULL amq_exchange_headers\n");\
    ...
    if (self->object_tag != AMQ_EXCHANGE_HEADERS_ALIVE) {\
        fprintf (stderr, "FATAL ERROR at %s:%u, in %s\n", __FILE__, __LINE__, ICL_ASSERT_SANE_FUNCTION);\
        fprintf (stderr, "amq_exchange_headers at %p expected object_tag=0x%x, actual object_tag=0x%x\n", self, AMQ_EXCHANGE_HEADERS_ALIVE, self->object_tag);\
