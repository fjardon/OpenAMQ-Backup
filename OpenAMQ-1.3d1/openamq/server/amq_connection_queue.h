...
{\
    if (!self) {\
        ...
        fprintf (stderr, "Attempting to work with a NULL amq_connection_queue\n");\
    ...
    if (self->object_tag != AMQ_CONNECTION_QUEUE_ALIVE) {\
        fprintf (stderr, "FATAL ERROR at %s:%u, in %s\n", __FILE__, __LINE__, ICL_ASSERT_SANE_FUNCTION);\
        fprintf (stderr, "amq_connection_queue at %p expected object_tag=0x%x, actual object_tag=0x%x\n", self, AMQ_CONNECTION_QUEUE_ALIVE, self->object_tag);\
