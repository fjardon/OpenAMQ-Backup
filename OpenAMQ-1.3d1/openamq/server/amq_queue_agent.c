...
#define set_last_binding_m ((amq_queue_agent_set_last_binding_t *) thread->method->data)
...
#define set_last_binding_m_event_name              "Set-Last-Binding-M"
...
#define set_last_binding_action_name               "Set-Last-Binding"
...
amq_queue_agent_set_last_binding_t;
...
#endif
#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_QUEUE_AGENT)  ||  defined (BASE_STATS_AMQ_QUEUE_AGENT_SET_LAST_BINDING))
static icl_stats_t *s_amq_queue_agent_set_last_binding_stats = NULL;
#endif
...
static int    action_set_last_binding         (smt_thread_t *thread);
...
static int
amq_queue_agent_set_last_binding_send (
    smt_method_queue_t *_queue,
    smt_event_t        _event,
    int            exchange_type,
    char *         routing_key,
    icl_longstr_t*  arguments);
int
amq_queue_agent_set_last_binding_destructor (void *data);
static int
    ...
    icl_longstr_t*  arguments)
{
    ...
    //  Possess code for set last binding
    ...
    _message = icl_mem_cache_alloc (s_set_last_binding_cache);
    _message->exchange_type             = exchange_type;
    ...
    rc = smt_method_send (_queue,           //  The queue
                          ...
                          SMT_OK,           //  Always OK
                          _message,         //  The message
                          amq_queue_agent_set_last_binding_destructor);
...
icl_longstr_t* arguments;
    ...
    exchange_type = ((amq_queue_agent_set_last_binding_t *) data)-> exchange_type;
    routing_key = ((amq_queue_agent_set_last_binding_t *) data)-> routing_key;
    arguments = ((amq_queue_agent_set_last_binding_t *) data)-> arguments;
    ...
    icl_longstr_t*        arguments)
{
    ...
    if (thread && !thread->zombie) {
        assert (thread->manager == amq_queue_agent_manager);
        rc = amq_queue_agent_set_last_binding_send (
                thread->reply_queue, set_last_binding_m_event,
                exchange_type,
                routing_key,
                arguments);
    }
...
amq_queue_agent_init (void)
{
    ...
    s_set_last_binding_cache = icl_cache_get (sizeof (amq_queue_agent_set_last_binding_t));
...
#endif
                    rc = action_set_last_binding (thread);
...
static int
action_set_last_binding (smt_thread_t *thread)
{
    int
        _rc = SMT_OK;

#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_QUEUE_AGENT)  ||  defined (BASE_STATS_AMQ_QUEUE_AGENT_SET_LAST_BINDING))
    icl_stats_inc ("amq_queue_agent_set_last_binding", &s_amq_queue_agent_set_last_binding_stats);
#endif        
#define tcb ((amq_queue_agent_class_context_t *) thread->context)
{
