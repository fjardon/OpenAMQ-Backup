...
#define protocol_unbind_queue_m ((amq_exchange_agent_protocol_unbind_queue_t *) thread->method->data)
...
enum {
    ...
    publish_m_event                                = 4,
    unbind_queue_m_event                           = 5,
    protocol_unbind_queue_m_event                  = 6,
    inspect_m_event                                = 7,
    modify_m_event                                 = 8,
    method_m_event                                 = 9,
};
...
#define protocol_unbind_queue_m_event_name         "Protocol-Unbind-Queue-M"
...
#define protocol_unbind_queue_action_name          "Protocol-Unbind-Queue"
...
amq_exchange_agent_protocol_unbind_queue_t;
...
static icl_cache_t
    *s_protocol_unbind_queue_cache = NULL;
static icl_cache_t
...
static const char *event_name [] = {
    ...
    protocol_unbind_queue_m_event_name,
...
static const char *action_name [] = {
    ...
    protocol_unbind_queue_action_name,
...
#endif
#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_EXCHANGE_AGENT)  ||  defined (BASE_STATS_AMQ_EXCHANGE_AGENT_PROTOCOL_UNBIND_QUEUE))
static icl_stats_t *s_amq_exchange_agent_protocol_unbind_queue_stats = NULL;
#endif
...
static int    action_protocol_unbind_queue    (smt_thread_t *thread);
...
static int
amq_exchange_agent_protocol_unbind_queue_send (
    smt_method_queue_t *_queue,
    smt_event_t        _event,
    amq_server_channel_t *  channel,
    amq_queue_t *  queue,
    ...
    icl_longstr_t *  arguments);
int
amq_exchange_agent_protocol_unbind_queue_destructor (void *data);
static int
    ...
    icl_longstr_t *  arguments)
{
    ...
    //  Possess code for protocol unbind queue
    ...
    _message = icl_mem_cache_alloc (s_protocol_unbind_queue_cache);
    _message->channel                   = channel;
    _message->queue                     = queue;
    _message->routing_key               = routing_key;
    _message->arguments                 = arguments;
    ...
    rc = smt_method_send (_queue,           //  The queue
                          ...
                          SMT_OK,           //  Always OK
                          _message,         //  The message
                          amq_exchange_agent_protocol_unbind_queue_destructor);
...
icl_longstr_t * arguments;
    ...
    channel = ((amq_exchange_agent_protocol_unbind_queue_t *) data)-> channel;
    queue = ((amq_exchange_agent_protocol_unbind_queue_t *) data)-> queue;
    routing_key = ((amq_exchange_agent_protocol_unbind_queue_t *) data)-> routing_key;
    arguments = ((amq_exchange_agent_protocol_unbind_queue_t *) data)-> arguments;
...
int
amq_exchange_agent_protocol_unbind_queue (
    smt_thread_t * thread,
    amq_server_channel_t * channel,
    amq_queue_t *         queue,
    char *                routing_key,
    icl_longstr_t *       arguments)
{
    ...
    if (thread && !thread->zombie) {
        assert (thread->manager == amq_exchange_agent_manager);
        rc = amq_exchange_agent_protocol_unbind_queue_send (
                thread->reply_queue, protocol_unbind_queue_m_event,
                channel,
                queue,
                routing_key,
                arguments);
...
amq_exchange_agent_init (void)
{
    ...
    s_protocol_unbind_queue_cache = icl_cache_get (sizeof (amq_exchange_agent_protocol_unbind_queue_t));
...
#undef  tcb
    ...
    icl_trace_record (thread->trace, s_dump, thread->id * 0x10000 + laststate + 5);
                ...
                case protocol_unbind_queue_m_event:
#if (defined (BASE_TRACE) || defined (BASE_TRACE_AMQ_EXCHANGE_AGENT))
    icl_trace_record (NULL, s_dump, thread->id * 0x10000 + 1);
...
#endif
                    rc = action_protocol_unbind_queue (thread);
...
static int
action_protocol_unbind_queue (smt_thread_t *thread)
{
    int
        _rc = SMT_OK;

#if (defined (BASE_STATS)  ||  defined (BASE_STATS_AMQ_EXCHANGE_AGENT)  ||  defined (BASE_STATS_AMQ_EXCHANGE_AGENT_PROTOCOL_UNBIND_QUEUE))
    icl_stats_inc ("amq_exchange_agent_protocol_unbind_queue", &s_amq_exchange_agent_protocol_unbind_queue_stats);
#endif        
#define tcb ((amq_exchange_agent_class_context_t *) thread->context)
{
...
icl_longstr_t *
    ...
    channel = protocol_unbind_queue_m->channel;
    queue = protocol_unbind_queue_m->queue;
    routing_key = protocol_unbind_queue_m->routing_key;
    arguments = protocol_unbind_queue_m->arguments;
        {
amq_queue_bindings_list_t
    *queue_bindings;                //  List of bindings for queue
amq_queue_bindings_list_iter_t *
