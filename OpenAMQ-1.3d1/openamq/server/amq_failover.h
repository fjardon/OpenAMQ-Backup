...
{\
    if (!self) {\
        ...
        fprintf (stderr, "Attempting to work with a NULL amq_failover\n");\
    ...
    if (self->object_tag != AMQ_FAILOVER_ALIVE) {\
        fprintf (stderr, "FATAL ERROR at %s:%u, in %s\n", __FILE__, __LINE__, ICL_ASSERT_SANE_FUNCTION);\
        fprintf (stderr, "amq_failover at %p expected object_tag=0x%x, actual object_tag=0x%x\n", self, AMQ_FAILOVER_ALIVE, self->object_tag);\
...
#   endif
//  Method prototypes
#define amq_failover_new(broker)        amq_failover_new_ (__FILE__, __LINE__, broker)
amq_failover_t *
    amq_failover_new_ (
...
#define amq_failover_new_in_scope(self_p,_scope,broker)  amq_failover_new_in_scope_ (self_p, _scope, __FILE__, __LINE__, broker)
void
