...
{\
    if (!self) {\
        ...
        icl_system_panic ("", "Attempting to work with a NULL amq_server_connection\n");\
        icl_system_panic ("", "Please report this to openamq-dev@lists.openamq.org\n");\
        abort ();\
    }\
    if (self->object_tag != AMQ_SERVER_CONNECTION_ALIVE) {\
        icl_system_panic ("", "FATAL ERROR at %s:%u, in %s\n", __FILE__, __LINE__, ICL_ASSERT_SANE_FUNCTION);\
        icl_system_panic ("", "amq_server_connection at %p expected object_tag=0x%x, actual object_tag=0x%x\n", self, AMQ_SERVER_CONNECTION_ALIVE, self->object_tag);\
