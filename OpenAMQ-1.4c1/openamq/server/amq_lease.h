...
{\
    if (!self) {\
        ...
        icl_system_panic ("", "Attempting to work with a NULL amq_lease\n");\
        icl_system_panic ("", "Please report this to openamq-dev@lists.openamq.org\n");\
        abort ();\
    }\
    if (self->object_tag != AMQ_LEASE_ALIVE) {\
        icl_system_panic ("", "FATAL ERROR at %s:%u, in %s\n", __FILE__, __LINE__, ICL_ASSERT_SANE_FUNCTION);\
        icl_system_panic ("", "amq_lease at %p expected object_tag=0x%x, actual object_tag=0x%x\n", self, AMQ_LEASE_ALIVE, self->object_tag);\
...
#   endif
//  Method prototypes
#define amq_lease_new(name,type,channel)  amq_lease_new_ (__FILE__, __LINE__, name, type, channel)
amq_lease_t *
    amq_lease_new_ (
...
#define amq_lease_new_in_scope(self_p,_scope,name,type,channel)  amq_lease_new_in_scope_ (self_p, _scope, __FILE__, __LINE__, name, type, channel)
void
