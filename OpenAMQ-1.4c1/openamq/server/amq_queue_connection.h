...
{\
    if (!self) {\
        ...
        icl_system_panic ("", "Attempting to work with a NULL amq_queue_connection\n");\
        icl_system_panic ("", "Please report this to openamq-dev@lists.openamq.org\n");\
        abort ();\
    }\
    if (self->object_tag != AMQ_QUEUE_CONNECTION_ALIVE) {\
        icl_system_panic ("", "FATAL ERROR at %s:%u, in %s\n", __FILE__, __LINE__, ICL_ASSERT_SANE_FUNCTION);\
        icl_system_panic ("", "amq_queue_connection at %p expected object_tag=0x%x, actual object_tag=0x%x\n", self, AMQ_QUEUE_CONNECTION_ALIVE, self->object_tag);\
