...
#    define AMQ_QUEUE_LIST_HISTORY_LENGTH 32
#endif
...
{\
    if (!self) {\
        ...
        icl_system_panic ("", "Attempting to work with a NULL amq_queue_list\n");\
        icl_system_panic ("", "Please report this to openamq-dev@lists.openamq.org\n");\
        abort ();\
    }\
    if (self->object_tag != AMQ_QUEUE_LIST_ALIVE) {\
        icl_system_panic ("", "FATAL ERROR at %s:%u, in %s\n", __FILE__, __LINE__, ICL_ASSERT_SANE_FUNCTION);\
        icl_system_panic ("", "amq_queue_list at %p expected object_tag=0x%x, actual object_tag=0x%x\n", self, AMQ_QUEUE_LIST_ALIVE, self->object_tag);\
...
struct _amq_queue_list_t {
    ...
    volatile qbyte
        history_last;
    char
        *history_file [AMQ_QUEUE_LIST_HISTORY_LENGTH];
    int
        history_line  [AMQ_QUEUE_LIST_HISTORY_LENGTH];
    char
        *history_type [AMQ_QUEUE_LIST_HISTORY_LENGTH];
    int
        history_links [AMQ_QUEUE_LIST_HISTORY_LENGTH];
#endif
    dbyte
...
#define amq_queue_list_link(self)       amq_queue_list_link_ (self, __FILE__, __LINE__)
amq_queue_list_t *
...
#define amq_queue_list_unlink(self)     amq_queue_list_unlink_ (self, __FILE__, __LINE__)
void
