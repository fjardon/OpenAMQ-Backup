/*---------------------------------------------------------------------------
    amq_exchange_regexp.c - amq_exchange_regexp component
...
amq_index_hash table.
    Generated from amq_exchange_regexp.icl by icl_gen using GSL/4.
...
#include "amq_exchange_regexp.h"        //  Definitions for our class

//  Shorthand for class type

#define self_t              amq_exchange_regexp_t

//  Shorthands for class methods

#define self_new            amq_exchange_regexp_new
#define self_annihilate     amq_exchange_regexp_annihilate
#define self_compile        amq_exchange_regexp_compile
#define self_publish        amq_exchange_regexp_publish
#define self_unbind         amq_exchange_regexp_unbind
#define self_selftest       amq_exchange_regexp_selftest
#define self_terminate      amq_exchange_regexp_terminate
#define self_show           amq_exchange_regexp_show
#define self_destroy        amq_exchange_regexp_destroy
#define self_alloc          amq_exchange_regexp_alloc
#define self_free           amq_exchange_regexp_free
#define self_cache_initialise  amq_exchange_regexp_cache_initialise
#define self_cache_purge    amq_exchange_regexp_cache_purge
#define self_cache_terminate  amq_exchange_regexp_cache_terminate
#define self_show_animation  amq_exchange_regexp_show_animation
#define self_new_in_scope   amq_exchange_regexp_new_in_scope

static void
    amq_exchange_regexp_annihilate (
amq_exchange_regexp_t * ( * self_p )    //  Reference to object reference
);

#define amq_exchange_regexp_alloc()     amq_exchange_regexp_alloc_ (__FILE__, __LINE__)
static amq_exchange_regexp_t *
    amq_exchange_regexp_alloc_ (
char * file,                            //  Source file for call
size_t line                             //  Line number for call
);

static void
    amq_exchange_regexp_free (
amq_exchange_regexp_t * self            //  Object reference
);

static void
    amq_exchange_regexp_cache_initialise (
void);

static void
    amq_exchange_regexp_cache_purge (
void);

static void
    amq_exchange_regexp_cache_terminate (
void);

Bool
    amq_exchange_regexp_animating = TRUE;  //  Animation enabled by default
static icl_cache_t
    *s_cache = NULL;
...
/*  -------------------------------------------------------------------------
    amq_exchange_regexp_new

    Type: Component method
    Creates and initialises a new amq_exchange_regexp_t object, returns a
    reference to the created object.
    -------------------------------------------------------------------------
 */

amq_exchange_regexp_t *
    amq_exchange_regexp_new_ (
    char * file,                        //  Source file for call
    size_t line,                        //  Line number for call
...
)
{
    ...
    self = amq_exchange_regexp_alloc_ (file, line);
    if (self) {
        self->object_tag   = AMQ_EXCHANGE_REGEXP_ALIVE;
#if defined (DEBUG)
        icl_mem_set_callback (self, amq_exchange_regexp_show_);
#endif
...
AMQ_EXCHANGE_REGEXP_ASSERT_SANE (self);
...
fprintf (trace_file, "    <amq_exchange_regexp file = \"%s\" line = \"%lu\"  pointer = \"%p\" />\n", file, (unsigned long) line, self);

}
/*  -------------------------------------------------------------------------
    amq_exchange_regexp_destroy

    Type: Component method
    Destroys a amq_exchange_regexp_t object. Takes the address of a
    amq_exchange_regexp_t reference (a pointer to a pointer) and nullifies the
...
if (self) {
    amq_exchange_regexp_annihilate (self_p);
    amq_exchange_regexp_free ((amq_exchange_regexp_t *) self);
    *self_p = NULL;
}
}
/*  -------------------------------------------------------------------------
    amq_exchange_regexp_alloc
...
self = (amq_exchange_regexp_t *) icl_mem_cache_alloc_ (s_cache, file, line);
if (self)
    memset (self, 0, sizeof (amq_exchange_regexp_t));
...
/*  -------------------------------------------------------------------------
    ...
    Type: Component method
    Freess a amq_exchange_regexp_t object.
    -------------------------------------------------------------------------
 */
...
if (self) {
        ...
        memset (&self->object_tag, 0, sizeof (amq_exchange_regexp_t) - ((byte *) &self->object_tag - (byte *) self));
//        memset (self, 0, sizeof (amq_exchange_regexp_t));
        self->object_tag = AMQ_EXCHANGE_REGEXP_DEAD;
        icl_mem_free (self);
    }
    self = NULL;
}
...
s_cache = icl_cache_get (sizeof (amq_exchange_regexp_t));
icl_system_register (amq_exchange_regexp_cache_purge, amq_exchange_regexp_cache_terminate);
}
/*  -------------------------------------------------------------------------
    amq_exchange_regexp_cache_purge
...
/*  -------------------------------------------------------------------------
    amq_exchange_regexp_show_animation
...
amq_exchange_regexp_animating = enabled;
}
/*  -------------------------------------------------------------------------
    amq_exchange_regexp_new_in_scope
...
void
    amq_exchange_regexp_new_in_scope_ (
    amq_exchange_regexp_t * * self_p,   //  Not documented
...
*self_p = amq_exchange_regexp_new_ (file,line,exchange);

if (*self_p) {
    _destroy = icl_destroy_new   ((void * *) self_p, (icl_destructor_fn *) amq_exchange_regexp_destroy_);
...
char *EMBED__amq_exchange_regexp_version_start  = "VeRsIoNsTaRt:ipc";
char *EMBED__amq_exchange_regexp_component  = "amq_exchange_regexp ";
char *EMBED__amq_exchange_regexp_version  = "1.0 ";
char *EMBED__amq_exchange_regexp_copyright  = "Copyright (c) 1996-2009 iMatix Corporation";
char *EMBED__amq_exchange_regexp_filename  = "amq_exchange_regexp.icl ";
char *EMBED__amq_exchange_regexp_builddate  = "2010/10/06 ";
char *EMBED__amq_exchange_regexp_version_end  = "VeRsIoNeNd:ipc";
