...
{\
    if (!self) {\
        ...
        icl_system_panic ("", "Attempting to work with a NULL amq_server_method\n");\
        icl_system_panic ("", "Please report this to openamq-dev@lists.openamq.org\n");\
        abort ();\
    }\
    if (self->object_tag != AMQ_SERVER_METHOD_ALIVE) {\
        icl_system_panic ("", "FATAL ERROR at %s:%u, in %s\n", __FILE__, __LINE__, ICL_ASSERT_SANE_FUNCTION);\
        icl_system_panic ("", "amq_server_method at %p expected object_tag=0x%x, actual object_tag=0x%x\n", self, AMQ_SERVER_METHOD_ALIVE, self->object_tag);\
...
#define AMQ_SERVER_RESTMS                   61501
#define AMQ_SERVER_RESTMS_PIPE_CREATE       10
#define AMQ_SERVER_RESTMS_PIPE_DELETE       20
#define AMQ_SERVER_RESTMS_FEED_CREATE       30
#define AMQ_SERVER_RESTMS_FEED_DELETE       40
#define AMQ_SERVER_RESTMS_JOIN_CREATE       50
#define AMQ_SERVER_RESTMS_JOIN_DELETE       60
...
typedef struct _amq_server_restms_pipe_create_t amq_server_restms_pipe_create_t;
typedef struct _amq_server_restms_pipe_delete_t amq_server_restms_pipe_delete_t;
typedef struct _amq_server_restms_feed_create_t amq_server_restms_feed_create_t;
typedef struct _amq_server_restms_feed_delete_t amq_server_restms_feed_delete_t;
typedef struct _amq_server_restms_join_create_t amq_server_restms_join_create_t;
typedef struct _amq_server_restms_join_delete_t amq_server_restms_join_delete_t;
#   ifdef __cplusplus
}
#   endif
...
struct _amq_server_restms_pipe_create_t {
    icl_shortstr_t
        pipe_type;                      //  pipe type
    icl_shortstr_t
...
struct _amq_server_restms_pipe_delete_t {
    icl_shortstr_t
        pipe_name;                      //  pipe name
};
...
struct _amq_server_restms_feed_create_t {
    icl_shortstr_t
        feed_type;                      //  Feed type
    icl_shortstr_t
...
struct _amq_server_restms_feed_delete_t {
    icl_shortstr_t
        feed_name;                      //  feed name
};
...
struct _amq_server_restms_join_create_t {
    icl_shortstr_t
        pipe_name;                      //  Name of pipe
    icl_shortstr_t
        feed_name;                      //  Name of feed
...
struct _amq_server_restms_join_delete_t {
    icl_shortstr_t
        pipe_name;                      //  Name of pipe
    icl_shortstr_t
        feed_name;                      //  Name of feed
